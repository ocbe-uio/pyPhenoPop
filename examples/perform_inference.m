...
% This function takes data stored in a csv file of dimensions (NR*NT, NC), i.e. all Time 0 replicates followed by all Time 1 replicates etc,
% and turns it into a 3D array of dimensions (NR,NC,NT) meaning NR rows down, NC columns right, NT outer dimension. 
...
%                       Their GR50 result will be in that unit. But if not in nanoMolar, the threshold ConcT should be changed to 
%                       a cencentration dose between the min and the max of the concentrations. 
% Time_array: Array containing the time points (in hours) in the drug screen data
% filename: a string specifying location of the data file with dimensions (NR*NT, NC)
% ----- Optional arguments: -----
...
% lower_bounds_optional: Lower bounds on parameters. The default are [-0.1, 0, 1e-6,   0, 1e-6]
% upper_bounds_optional: Higher bounds on parameters. The default are [ 0.1, 1,  2e3, 100, 5e4]
...
function [x_finals_temp, f_vals_temp, negative_loglikelihood_values] = perform_inference(experiment_name, NR, Concentration_array, Time_array, filename, max_no_populations_optional, num_optim_optional, lower_bounds_optional, upper_bounds_optional, USE_TWO_NOISE_LEVELS_optional, ConcT_optional, TimeT_optional) 
    tStart = tic;
    ...
    % Read data
    flat_data = table2array(readtable(filename));
